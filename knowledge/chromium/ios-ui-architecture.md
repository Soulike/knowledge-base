...
The conventional legacy skeleton under `ios/chrome/browser/<feature>/` centers on two coarse responsibility buckets. Current Chromium changes explicitly describe `ui_bundled/` as legacy. The examples below come from Bookmarks and show representative contents rather than a required or exhaustive inventory.

| Directory     | Responsibility                                                                                                                                                                                                                                       | Representative Bookmarks contents                                                                                                                                                                                                                                                            |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model/`      | Non-UI feature state, services, factories, persistence, rules, and model-facing utilities. Unit tests normally remain beside the implementation they cover.                                                                                          | Service and model factories such as `bookmark_model_factory.*` and `account_bookmark_sync_service_factory.*`; model observers and bridges such as `bookmark_model_bridge_observer.*`; `bookmarks_utils.*`; and co-located `*_unittest.*` files.                                              |
| `ui_bundled/` | The feature's aggregate UI and integration code. A legacy directory can contain coordinators, mediators, view controllers, views, UI-facing protocols, resources, and tests together rather than expressing those responsibilities in its top level. | Top-level files such as `bookmark_mediator.*` and `bookmark_navigation_controller.*`; `home/` containing `bookmarks_coordinator.*`, `bookmarks_home_mediator.*`, `bookmarks_home_view_controller.*`, and its consumer protocol; plus `cells/`, `resources/`, unit tests, and EarlGrey tests. |
...
Current Chromium refactors replace the aggregate `ui_bundled/` directory with directories that expose architectural ownership at the feature root. Manual Fill provides a complete example of this layout:

| Directory      | Responsibility                                                                                                                                                                                                              | Representative Manual Fill contents                                                                                                                                                                                      |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `model/`       | Non-UI feature state, services, factories, persistence, rules, and model-facing utilities.                                                                                                                                  | Feature value and adapter types such as `manual_fill_address.*`, `manual_fill_credential.*`, and `manual_fill_credit_card.*`; model-facing helpers such as `manual_fill_injection_handler.*`; and co-located unit tests. |
| `coordinator/` | Coordinators, mediators, and their private protocols or bridges. This directory owns feature composition, presentation lifecycle, model observation, and adaptation between model and UI semantics.                         | Flow coordinators such as `address_coordinator.*` and `card_coordinator.*`; `manual_fill_*_mediator.*`; coordinator delegates and bridges; and coordinator or mediator unit tests.                                       |
| `ui/`          | View controllers, views, cells, other UIKit elements, and UI-facing protocols such as consumers and mutators. Its interfaces use UI-ready values rather than exposing model objects.                                        | `address_view_controller.*`, `card_view_controller.*`, consumer and list-delegate protocols, reusable cells and buttons, and co-located view or component unit tests.                                                    |
| `public/`      | The deliberately exposed feature API needed outside a private implementation directory or across a feature boundary. Keep this surface narrow; implementation convenience alone does not make a declaration public.         | The small externally consumed surface: `manual_fill_constants.*` and `manual_fill_content_injector.h`.                                                                                                                   |
| `test/`        | Integration or end-to-end tests and test-only support that should not form part of a production API. Unit tests may remain beside the implementation they cover, and some features retain a separate `eg_tests/` directory. | EarlGrey feature tests such as `address_view_controller_egtest.mm` and `fallback_coordinator_egtest.mm`, together with test-only matchers in `manual_fill_matchers.*`.                                                   |
...
| Directory             | Responsibility                                                                                                                        | Representative contents                                                                                                                                                                               |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shared/model/`       | Code shared only by model objects.                                                                                                    | `application_context/`, `browser/` with `Browser` and `BrowserList`, `profile/`, `prefs/`, and `web_state_list/`.                                                                                     |
| `shared/coordinator/` | Code shared only by coordinators or mediators.                                                                                        | `chrome_coordinator/` with the common coordinator base, plus reusable `root_coordinator/`, `scene/`, `alert/`, and `layout_guide/` coordination code.                                                 |
| `shared/ui/`          | Code shared by UI elements; UI-related code also used by coordinators or mediators belongs here rather than in `shared/coordinator/`. | Reusable UI libraries such as `elements/`, `table_view/`, `symbols/`, `resources/`, and `util/`; `elements/`, for example, contains shared views, view controllers, buttons, labels, and their tests. |
| `shared/public/`      | API shared across all architectural audiences.                                                                                        | `commands/` with `CommandDispatcher` and cross-feature command protocols, plus shared `features/`, `metrics/`, and `snackbar/` APIs.                                                                  |
