...
This authority covers one transition of a verified pull request from draft to
ready. It does not cover conversion back to draft, requesting reviewers,
approving, enabling auto-merge, or merging.

Grant one readiness transition attempt per accepted review-progression intent.
Record the complete ready-for-review and convert-to-draft event history as the
authority baseline and record whether the attempt has been consumed. Invoking
the provider operation consumes the authority regardless of success, failure,
or an unknown result. Preserve that state across workflow boundaries. A later
consumer under the same accepted request inherits the consumed state. A resumed
workflow or a request merely to continue watching does not renew it; only a new
explicit user instruction may authorize another attempt.

## Gate readiness

Before publishing a ready state, require all of these conditions:

1. The pull request is open and draft. Its identity, canonical repository,
   target repository, ref, and expected SHA, source repository, ref, and
   expected SHA, and complete draft-transition history are fixed and still
   match the captured state and accepted request. The one-shot authority remains
   unconsumed.
...
Keep the pull request as a draft and report the blocking condition when any
requirement does not hold.

## Publish once and verify the result

Some providers, including GitHub, cannot atomically bind a ready-for-review
transition to the expected source SHA. This workflow accepts the resulting
bounded check-then-act race only after the readiness gate passes.
