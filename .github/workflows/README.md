...
The custom job named exactly `AI review gate` remains the repository's required check. The [gate implementation](../scripts/ai-review/review-gate.ts) accepts exactly one `github-actions[bot]` `COMMENTED` review only when its API commit, visible model/verdict/count/head fields, gh-aw-owned attribution, current pull-request state, and current run-attempt `safe_outputs` time window agree. `high` or `medium` findings select `needs-change`; otherwise the verdict is `approved`.
