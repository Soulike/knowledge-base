...
The gate owns `AI Approved` and `AI Need Change`. It clears both labels for missing, failed, malformed, stale, draft, or untrusted review paths while the pull request remains open. After applying a verdict label, it re-reads the pull request, reviews, and current-attempt jobs. A head change removes the newly applied label. The workflow does not run for pull-request closure, and an in-flight gate that observes closure before or after a cleanup attempt preserves or restores the preceding verdict label. A `needs-change` review and label remain visible while the required check fails.
