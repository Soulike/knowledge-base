...
For example, provider and model-selection metadata may recognize a model while the client uses generic capacity defaults. A successful request then proves callability, not that an expected larger context tier was used. Conversely, a model with known client configuration can still be unavailable to the workflow's credential. Treat model names as candidates to evaluate, not a portable whitelist.

The implementation details below were checked on 2026-09-05 against gh-aw v0.87.10/v0.88.2, AWF v0.28.10/v0.28.12, and Copilot CLI v1.0.80/v1.0.83. The catalog and capacity example was checked on 2026-09-12 against gh-aw v0.88.7 and checksummed Copilot CLI v1.0.79/v1.0.80/v1.0.83 release packages. These are evidence boundaries, not recommended version pins or a promise about every later release.
...
| Copilot CLI model-selection metadata      | Models recognized or presented by that client version                                                   | That BYOK numeric-limit resolution has matching model configuration      |
...
A direct inspection demonstrates that these sources can disagree. Copilot CLI v1.0.79 and v1.0.80 recognized `gpt-5.6-sol` in model-selection metadata, but their BYOK numeric-limit catalog reported the identifier absent and returned no model limits. Known-model controls queried through the same interface returned numeric prompt, context-window, and output limits. Without a matching entry or explicit overrides, the inspected resolver used generic limits of 128,000 prompt tokens and a 200,000-token context window, with no model-specific maximum-output value. [Copilot CLI v1.0.79 release](https://github.com/github/copilot-cli/releases/tag/v1.0.79), [Copilot CLI v1.0.80 release](https://github.com/github/copilot-cli/releases/tag/v1.0.80).

At the same time, gh-aw v0.88.7 included `gpt-5.6-sol` with pricing, provider type, and `wire_api: responses`; its Copilot harness consumed the wire-API field but had no catalog capacity fields to apply to the CLI. This proves catalog disagreement and the resulting client fallback, not provider entitlement or actual provider capacity. [gh-aw model metadata](https://github.com/github/gh-aw/blob/v0.88.7/actions/setup/js/models.json#L498-L506), [Copilot harness](https://github.com/github/gh-aw/blob/v0.88.7/actions/setup/js/copilot_harness.cjs#L443-L483).

When inspecting a client catalog, query at least one known model through the same interface as a positive control. A missing candidate result is meaningful only when the control shows that the catalog and probe are functioning. Copilot CLI v1.0.83 removed the earlier exposed numeric-limit lookup while retaining model-selection metadata for `gpt-5.6-sol`; the inspected interfaces therefore did not establish that version's numeric capacity behavior. Keep such behavior unresolved rather than carrying forward an earlier fallback or assuming that model recognition supplies the missing limits. [Copilot CLI v1.0.83 release](https://github.com/github/copilot-cli/releases/tag/v1.0.83).
