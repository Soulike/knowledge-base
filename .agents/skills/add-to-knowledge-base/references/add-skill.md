...
1. Choose the Skill audience before choosing its path:
   ...
   - Put an independent plugin's usage workflows inside that plugin's own
     `skills/` directory.
2. Read
   [Agent Skill authoring](../../../../knowledge/agents/skill-authoring.md) and
...
4. Before adding or changing a repository-authoring or primary-plugin Skill,
   ...
   Skill body or its `references/`. Return missing material to the parent
   workflow as mixed only when it has an independent direct-reading
   responsibility outside the workflow, then complete the Knowledge branch
   first.
...
9. Run repository formatting and relevant tests. Confirm that the Skill is in
